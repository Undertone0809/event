		...
		<scroll-view id="_scroll" :scroll-x="true" class="scroll-view-h" scroll-with-animation :scroll-left="slider.scrollLeft">
			<view class="_scroll-content">
				<view class="_tab-item-box" :class="[defaultConfig.itemWidth ? '_clamp' : '_flex']">
					<block v-for="(item, index) in tabList" :key="index" >
						<view 
							...
							:style="{color: tagIndex == index ? defaultConfig.activeColor : defaultConfig.color, 'width': defaultConfig.itemWidth ? defaultConfig.itemWidth + 'rpx' : '',fontWeight:tagIndex == index ?900:500}"
				...
				<view class="_underline" :style="{
						...
						backgroundImage: defaultConfig.underLineColor,
					}" />
				</view>
		...
		data() {
			return {
				...
				defaultConfig: {
					...
					underLineHeight: 4,
					// 下划线颜色
					underLineColor: 'linear-gradient(270deg, #24fbe7 0%, #669cff 100%)',
				},
			};
		},
		watch: {
